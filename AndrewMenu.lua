        ...
        while ConfigBurla.RepetirSiempre and BucleBurlaActivo do
            ...
            if ConfigBurla.MostrarMensajes then
                if exito then
                    ...
                    print("❌ Bloqueado: " .. tostring(error))
                end
            end
        end
    end)
...
-- VELOCIDAD: ✅ GUARDA Y RESPETRA LA VELOCIDAD ORIGINAL DEL JUEGO
local function applySpeed()
    ...
    if playerState.isSpeedBoosted and not playerState.isFlying then
        ...
        if playerState.originalSpeed then
            humanoid.WalkSpeed = playerState.originalSpeed
        end
        -- Si no hay velocidad original guardada, NO TOCAMOS NADA
    end
end
local function toggleSpeedBoost()
    ...
    -- ✅ Guardamos la velocidad real del juego la primera vez
    if not playerState.originalSpeed then
...
-- ⭐ AL RESPAWNEAR: Guarda velocidad original si falta, vuelo se apaga, burla queda activa
player.CharacterAdded:Connect(function(newChar)
    task.wait(0.5)
    local hum = newChar:FindFirstChildOfClass("Humanoid")
    if hum and not playerState.originalSpeed then
...
-- ✅ GUARDAR VELOCIDAD AL CARGAR EL SCRIPT POR PRIMERA VEZ
task.wait(1.5)
local initialHum = getHumanoid()
if initialHum then
    playerState.originalSpeed = initialHum.WalkSpeed
end
-- Noclip continuo
